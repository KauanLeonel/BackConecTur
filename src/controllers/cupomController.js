...
export async function createCupom(req, res) {
  ...
  if (isNaN(lojaId) || !descricao || !dataValidade) {
    return res
      ...
      .json({
        message:
          "ID da loja, descrição e data de validade do cupom são obrigatórios.",
      });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao criar cupom", error: error.message });
  }
}
...
export async function getAllCupons(req, res) {
  ...
  try {
    const cupons = await prisma.cupom.findMany({
      ...
      include: {
        loja: {
          // Inclui os dados da loja associada
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar cupons", error: error.message });
  }
}
...
export async function getCupomById(req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar cupom", error: error.message });
  }
}
...
export async function updateCupom(req, res) {
  ...
  if (dataValidade !== undefined)
    dataToUpdate.dataValidade = new Date(dataValidade);
  ...
  if (lojaId !== undefined) {
    if (isNaN(lojaId)) {
      return res
        .status(400)
        .json({ message: "ID da loja inválido para reassociar." });
    }
  ...
  if (Object.keys(dataToUpdate).length === 0) {
    return res
      .status(400)
      .json({ message: "Nenhum dado fornecido para atualização do cupom." });
  }
  ...
  try {
    ...
    if (error.code === "P2025") {
      ...
      return res
        .status(404)
        .json({ message: "Cupom não encontrado para atualização." });
    }
    res
      .status(500)
      .json({ message: "Erro ao atualizar cupom", error: error.message });
  }
}
...
export async function deleteCupom(req, res) {
  ...
  try {
    ...
    if (error.code === "P2025") {
      return res
        .status(404)
        .json({ message: "Cupom não encontrado para exclusão." });
    }
    res
      .status(500)
      .json({ message: "Erro ao deletar cupom", error: error.message });
  }
}
...
export async function resgatarCupom(req, res) {
  ...
  if (isNaN(cidadaoId) || isNaN(cupomId)) {
    return res
      .status(400)
      .json({ message: "IDs de cidadão ou cupom inválidos." });
  }
  ...
  try {
    ...
    const resgateExistente = await prisma.cidadaoCupomResgatado.findUnique({
      where: {
        cidadao_id_cupom_id: { cidadao_id: cidadaoId, cupom_id: cupomId },
      },
    });
    if (resgateExistente) {
      return res
        .status(409)
        .json({ message: "Cupom já resgatado por este cidadão." });
    }

    const novoResgate = await prisma.cidadaoCupomResgatado.create({
      data: {
    ...
    res.json({
      ...
      cupom: cupom /*, resgate: novoResgate */,
    });
  } catch (error) {
    console.error("Erro ao resgatar cupom:", error);
    res
      .status(500)
      .json({ message: "Erro ao resgatar cupom", error: error.message });
  }
}
